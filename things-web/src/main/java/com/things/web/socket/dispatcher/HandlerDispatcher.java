package com.things.web.socket.dispatcher;
...
import com.things.common.assembly.AssemblyManage;
import com.things.web.socket.domain.GameRequest;
import com.things.web.socket.domain.GameResponse;
import com.things.web.socket.domain.MessageQueue;
import com.things.web.socket.handler.GameHandler;
import com.things.web.socket.utils.ExceptionUtils;
import com.things.web.socket.utils.HttpUtils;
...
public class HandlerDispatcher implements Runnable {
	...
	private final class MessageWorker implements Runnable {
		...
		private void handMessageQueue() {
			String messageId = this.request.getCommandId();
			GameResponse response = new GameResponse(this.request.getChannel(), this.request.getCommand(), this.request.getRequestType());
			GameHandler handler = AssemblyManage.<GameHandler>getBean(String.format("socket.%s",messageId),GameHandler.class);
			if (handler != null)
